...
namespace MISA.CUKCUK.Core.Services
{
    public class BaseService<MISAEntity> : IBaseService<MISAEntity> where MISAEntity : class
    {
        ...
        protected bool Validate(Guid? entityId, MISAEntity entity)
        {
            ...
            foreach (var property in properties)
            {
                ...
                bool isRequired = Attribute.IsDefined(property, typeof(MISARequired));
        ...
        public PaggingResult<MISAEntity> GetPaging(int pageSize, int pageNumber, List<Filter> filter)
        {
            ...
            if (filter != null)
            {
                foreach (var property in properties)
                {
                    // Kiểm tra column có được filter không
                    bool isFilter = Attribute.IsDefined(property, typeof(MISAFilter));

                    if (isFilter)
                    {
                        foreach (var filterValue in filter)
                        {
                            if (filterValue.ColumnName == property.Name)
                            {
                                var filterAttribute = Attribute.GetCustomAttribute(property, typeof(MISAFilter));

                                string prefix = (filterAttribute as MISAFilter).Prefix;

                                switch (filterValue.Calculation)
                                {
                                    case Enum.FilterCalculation.EQual:
                                        filterStr += String.Format(Resources.ResourceVN.FilterEqual, prefix, property.Name, filterValue.Value);
                                        break;
                                    case Enum.FilterCalculation.Contain:
                                        filterStr += String.Format(Resources.ResourceVN.FilterContain, prefix, property.Name, filterValue.Value);
                                        break;
                                    case Enum.FilterCalculation.Greater:
                                        filterStr += String.Format(Resources.ResourceVN.FilterGreater, prefix, property.Name, filterValue.Value);
                                        break;
                                    case Enum.FilterCalculation.GreaterOrEqual:
                                        filterStr += String.Format(Resources.ResourceVN.FilterGreaterOrEqual, prefix, property.Name, filterValue.Value);
                                        break;
                                    case Enum.FilterCalculation.Less:
                                        filterStr += String.Format(Resources.ResourceVN.FilterLess, prefix, property.Name, filterValue.Value);
                                        break;
                                    case Enum.FilterCalculation.LessOrEqual:
                                        filterStr += String.Format(Resources.ResourceVN.FilterLessOrEqual, prefix, property.Name, filterValue.Value);
                                        break;
                                    case Enum.FilterCalculation.NotEqual:
                                        filterStr += String.Format(Resources.ResourceVN.FilterNotEqual, prefix, property.Name, filterValue.Value);
                                        break;
                                    default:
                                        break;
                                }
            ...
            PaggingResult<MISAEntity> res = _baseRepository.GetPaging(filterStr, pageSize, pageNumber);
            return res;
        }
    }
}
